defmodule ConfigComparator.SysconfigReader do
  ...
  def handle_call({:get, {key1, key2}}, _from, state =
                              {{:sysconfig, config}, _}) do
    value = config
    |> Enum.into(%{})
    |> Map.get(key1)
    |> Enum.into(%{})
  ...
  def handle_call({:diff, exs_config}, _fromt, _state =
                              {{:sysconfig, config}, _}) do
  ...
  defp read(path, true) do
    ...
    sysconfig_str =
      :lists.append(:string.tokens(:erlang.binary_to_list(config), '\n '))
    ...
    |> Enum.reduce(a,
        ...
        case compare(value, get_value(key, config)) do
          ...
          {:mismatch, val1 = [{_, _}|_tail], val2} ->
                  [{_, missing2}, {_, diff2}] =
                                find_diffs(val1, val2, acc, keys(k, key))
                                [{:missing, :lists.append(missing, missing2)},
                                {:diff, :lists.append(diff, diff2)}]
          {:mismatch, val1, val2} ->
